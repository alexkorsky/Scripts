...
def estimate_volatility(prices, l):
    ...
    square_log_returns = np.square(np.log(prices) - np.log(prices.shift(1))) 
    ewm_mean = square_log_returns.ewm(alpha=1-l).mean() 
    ewm_volatility = np.sqrt(ewm_mean) 
    
    print (ewm_volatility)
    return ewm_volatility.iloc[-1]

    
def test_run(filename='volatilityestimatedata.csv'):
